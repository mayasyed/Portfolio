# Mahanoor Shams Portfolio

Works as a Data Analyst focused on improving operational efficiency, data accuracy, and reporting reliability across complex business environments. Experienced in analysing large datasets within ERP systems, identifying data quality issues, and ensuring data is clean, consistent, and reliable for decision-making.

Uses SQL to extract, validate, and analyse data, and builds dashboards and reports using Power BI and Looker to provide clear, actionable insights. Strong background in data cleaning, data reporting, and data analysis, with a focus on transforming messy operational data into meaningful business insights for stakeholders.

Collaborates closely with operations, and cross-functional teams to improve reporting processes, reduce manual effort, and support data-driven decisions. Actively develops skills in Python, analytics best practices, and modern data tools, with a mindset of continuous learning and process improvement.

# Skills

# Projects

## Links 
- Linkedin
- Tableau
